...
# Tightened to reject noise that was producing 3-4 false hits per real block.
# S floor 110 = must be VIVID red (rejects wood grain, skin, dim cables).
# V floor 70  = must be reasonably bright (rejects dark red shadows).
# If your blocks look orangey, drop UPPER_1 hue to 8. If pinkish, raise
# LOWER_2 to 165.
RED_HSV_LOWER_1 = (0,   110, 70)
RED_HSV_UPPER_1 = (10,  255, 255)
RED_HSV_LOWER_2 = (170, 110, 70)
RED_HSV_UPPER_2 = (180, 255, 255)
# Min area: 250 px ≈ 16x16 pixel blob. A real block at our camera distance
# is much bigger (~50x50+), so this rejects almost all noise without
# rejecting real blocks. Bump UP if you still see false hits, DOWN if real
# blocks are being missed.
TARGET_MIN_AREA = 250
TARGET_MAX_AREA = 20000      # rejects huge reds (clothing, big background items)
# After two real detections' centroids land within this many ROBOT MILLIMETRES
# of each other, treat them as the same block (keeps the bigger one). Stops
# specular highlights / partial occlusions from being counted twice.
DEDUPE_DIST_MM = 25
# Cap on how many targets we'll act on per run. Helps reject any noise that
# slips through — pick at most this many of the LARGEST detections.
MAX_TARGETS = 6
# Show a "Red Mask (debug)" window so you can see exactly what the
# thresholds are accepting. Set False to hide.
RED_DEBUG_MASK = True
...
def find_red_objects(frame, draw_on=None):
    ...
    Pipeline:
      1. Blur → HSV → two hue-band threshold (red wraps at H=0).
      2. OPEN 3x3 to kill specks, CLOSE 9x9 + dilate 5x5 to fuse highlight
         splits on a single block back into one contour.
      3. Restrict to work-area (caution minus danger).
      4. Drop tiny/huge contours.
      5. Dedupe near-duplicate centroids in ROBOT mm (DEDUPE_DIST_MM).
      6. Keep at most MAX_TARGETS largest blobs.
    ...
    # OPEN first to delete salt-and-pepper specks.
    mask = cv2.morphologyEx(mask, cv2.MORPH_OPEN,  np.ones((3, 3), np.uint8))
    # CLOSE with a bigger kernel + small dilate stitches glossy-block fragments
    # (specular highlights, label gaps) back into one contour.
